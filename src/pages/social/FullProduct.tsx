...
const FullProduct = () => {
  ...
  const _saveIntroduction = (item: string) => {
    // console.log(item);
    setActiveId2(item);
    setData({ ...postData, features: item });
    toast.info('Click Next to proceed ahead', {
      position: "top-center",
      autoClose: 1000,
      hideProgressBar: true,
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (form.checkValidity() === false) {
      ...
      const data = await apiService.contentPost('caiproducttitle', { searchData });
  ...
  const handleSubmit1 = async (e: any) => {
    ...
    if (form.checkValidity() === false) {
      ...
      setsearchData1([]);
      const searchData = postData;
      const data = await apiService.contentPost('caiproductfeatures', { searchData });
      if (data.success === false) {
        toast.info(data.Message, {
          position: "top-right",
          autoClose: 1000,
  ...
  const handleSubmit2 = async (e: any) => {
    ...
    if (form.checkValidity() === false) {
      ...
      setsearchData2([]);
      const searchData = postData;
      const data = await apiService.contentPost('caiproductdesc', { searchData });
      if (data.success === false) {
        toast.info(data.Message, {
          position: "top-right",
          autoClose: 1000,
  ...
  const handleKeywords = async (e: any) => {
    ...
    setkeywordData([]);
    const searchData = postData;
    const data = await apiService.contentPost('caiproducttags', { searchData });
    if (data.success === false) {
      toast.info(data.Message, {
        position: "top-right",
        autoClose: 1000,
  ...
  const handleSubmit3 = async (e: any) => {
    ...
    if (form.checkValidity() === false) {
      ...
      setsearchData3([]);
      const searchData = postData;
      const data = await apiService.contentPost('caigetproductcard', { searchData });
      if (data.success === false) {
        toast.info(data.Message, {
          position: "top-right",
          autoClose: 1000,
  ...
  const getTitle = async () => {
    ...
    setTitleName("Full Product Wizard");
    setTitleCard("GeCreate high-converting product cards that differentiate your products from the masses.");
    setDescPlace("eg. Listen all day long with wireless headphones that deliver crystal-clear sound and come with noise-cancellation, modern design, and long battery life.");
  };
                  ...
                  <React.Fragment>
                    ...
                    <Row>
                      {(activities || []).map((item, index) => {
                        return (
                          <Col key={index}>
                            {/* <div>{steps.indexOf(step) < 0 ? 'sucess' : 'fail'}</div> */}
                          ...
                          <Form noValidate validated={validated2} onSubmit={handleSubmit2}>
                            <Form.Group className="mb-1" controlId="validationCustom09">
                              <Form.Label className="w-100">
                                ...
                                {/* <Button className="float-end" type="submit" onClick={handleKeywords} disabled={isSubmitting4}>
                                  {isSubmitting4 && (
                                    <span className="spinner-border spinner-border-sm me-1"></span>
                                  )}
                                  Generate Keywords
                        ...
                        render={({ next, previous }) => (
                          <Form noValidate validated={validated2} onSubmit={handleSubmit2}>
                            <Form.Group className="mb-1" controlId="validationCustom07">
                              <Form.Label>Product Topic</Form.Label>
                              <Form.Control
                                key="title"
                                name="title"
                                onChange={handleChange}
                      ...
                      <Step
                        ...
                        render={({ previous }) => (
                          <Form noValidate validated={validated3} onSubmit={handleSubmit3}>
                            ...
                            <Form.Group className="mb-1" controlId="validationCustom09">
                              <Form.Label>Product Outline</Form.Label>
                              <Form.Control
                                as="textarea"
                                key="outline"
                                name="outline"

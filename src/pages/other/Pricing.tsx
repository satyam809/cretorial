...
const PricingCard = ({ pricingPlans }: PricingCardProps) => {
    ...
    const [subsvalidityNew, setsubsvalidity] = useState("");
    const [planidNew, setplanid] = useState("");
    const [subslimitNew, setsubslimit] = useState("");
    ...
    const handlePayment = async (plan: any) => {
        ...
        if (plan.subsprice === 0) {
            ...
            const data = await apiService.caisubspayment({ postData });
            if (data) {
                toast.success(`Trial Start Successfully`, {
                    position: "top-right",
                    autoClose: 5000,
            ...
            const data = await apiService.postSubscription(plan.planname, plan.subsid);
            // console.log(data);
            setsubsvalidity(plan.subsvalidity);
            setplanid(data.subid);
            setsubslimit(plan.subslimit);
            ...
            if (data) {
                const options = {
                    ...
                    handler: async function (transaction: any) {
                        console.log(plan.subsvalidity);
                        console.log(data.subid);
                        console.log(plan.subslimit);
                        ...
                        // console.log(postData);
                        await apiService.updateSubscription(data.subid);
                        const newdata = await apiService.caisubspayment({ postData });
                        if (newdata) {
    ...
    const getPlans = async () => {
        const data = await apiService.caisubscriptions();
        // console.log(data);
        setplansData(data.subscritions);
                            ...
                            <Card.Body className="px-3 py-4">
                                <div className="d-flex">
                                    <div className="flex-grow-1">
                                        ...
                                        <h2 className="mt-0 mb-2">
                                            {plan.subsprice === 0 ?
                                                <>
                                                    Free {plan['subsvalidity']} Days
                                                </>
                                                :
                                                <>
                                                    ₹{plan.subsprice} <span className="fs-14">/ {plan['subsvalidity']} Days</span>
                                                </>
                                            }
                                ...
                                <div className="mt-5 text-center">
                                    ...
                                    <Button onClick={() => handlePayment(plan)}
                                        ...
                                        {plan.subsprice === 0 ?
                                            <>
                                                <i className="uil uil-arrow-right me-1"></i>Try Now
                                            </>
                                            :
                                            <>
